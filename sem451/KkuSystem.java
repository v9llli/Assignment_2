...
public class KkuSystem implements FileNames, ReserveTasks, OptionalReserveTasks{
	...
	public void showCLIMenu() {
		...
		Person p ;
                LocalDate l ;
                int t; Room r; boolean st; Scanner sc = new Scanner(System.in); String s; 
		...
		while(true) {
			...
			System.out.println("9. Remove User");
                        System.out.println("10. Find reservations by Person");
			...
			case "1":
			    try{
                                ...
                                a2 = sc.next();
                                a3 = sc.nextInt();
                                p = new Person(a1,a2,a3);
                                if (a1.length() >= 2 & a3 >=0 ){
                                    people.addPerson(p); }
                                else {
                                    System.out.println("Enter again person name with (greater than 1 letter), id, age greater than 0 (press Enter after each) \n:");
                                    a1 = sc.next();
                                    a2 = sc.next();
                                    a3 = sc.nextInt();
                                }
				...
				l=LocalDate.parse(sc.next());
                                    ...
                                    System.out.println("Sir, that's wrong .. please Enter correct date, for example: '2024-02-09'");
                                    l=LocalDate.parse(sc.next());
                                }
				System.out.println("At what Clock 1-24 (Only 1 hour can be booked)?");
				t=sc.nextInt();
                                while(t > 24) {
                                    System.out.println("Please enter a correct Clock 'from 0 to 24' !!ONLY!! ");
                                    t=sc.nextInt();
			...
			case "9":
				...
				break;
                        ...
                        case "10":
                                System.out.println("Enter Person 'Name, ID , Age' to display:");
                                String s1 = sc.next(); String s2 = sc.next(); int s3 = sc.nextInt();
